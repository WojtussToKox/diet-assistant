...
export default function RecipesPanel({ toast }) {
  ...
  const openEdit = async (r) => {
    try {
      ...
      setForm({
        ...
        ingredients: d.ingredients.map(i => ({ product: String(i.product), weight_in_grams: String(i.weight_in_grams) })),
      });
      setModal(d);
  ...
  const calcNutrition = (ingredients) => {
    ...
    ingredients.forEach(ing => {
      ...
      cal += Number(prod.calories_per_100g) * w;
      pro += Number(prod.protein_per_100g) * w;
      fat += Number(prod.fat_per_100g) * w;
      carb += Number(prod.carbs_per_100g) * w;
    });
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        ingredients: form.ingredients.map(i => ({ product: Number(i.product), weight_in_grams: Number(i.weight_in_grams) })),
      };
      if (modal === "add") {
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      toast("Przepis usunięty", "success"); reload();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="m-0 text-2xl font-bold text-text">Przepisy</h2>
          <p className="m-0 mt-1 text-text-muted text-sm">{recipes.length} przepisów</p>
        </div>
            ...
            <Card key={r.id} hover onClick={() => openDetail(r)}>
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="font-bold text-base text-text">{r.name}</div>
                  {r.description && <div className="text-[13px] text-text-muted mt-1 leading-relaxed">{r.description.slice(0, 120)}{r.description.length > 120 ? "…" : ""}</div>}
                </div>
                <div className="flex gap-1.5 ml-3" onClick={e => e.stopPropagation()}>
      ...
      {detail && (
        <Modal title={detail.name} onClose={() => setDetail(null)} width="max-w-[600px]">
          {detail.description && <p className="text-text-muted text-sm mt-0 mb-4">{detail.description}</p>}
          <h4 className="text-sm font-semibold text-text mb-2.5">Składniki</h4>
          <div className="grid gap-1.5 mb-5">
            {(detail.ingredients || []).map((ing, i) => (
              <div key={i} className="flex justify-between py-2 px-3 bg-background rounded-lg text-sm">
                <span className="text-text">{ing.product_name}</span>
                <span className="text-accent font-semibold font-mono">{ing.weight_in_grams}g</span>
              </div>
            ))}
          </div>
          {(() => {
            ...
            return (
              <div className="p-4 bg-background rounded-xl">
                <div className="text-xs text-text-muted mb-2">Wartości odżywcze (cały przepis)</div>
        ...
        <Modal title={modal === "add" ? "Nowy przepis" : "Edytuj przepis"} onClose={() => setModal(null)} width="max-w-[640px]">
          <div className="grid gap-3.5">
            ...
            <label className="block">
              <span className="block text-[13px] font-semibold text-text mb-1">Opis / sposób przygotowania</span>
              <textarea 
                value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="Opcjonalny opis przepisu…" rows={3}
                className="w-full box-border px-3 py-2 text-sm border-[1.5px] border-border rounded-lg bg-surface text-text font-inherit outline-none resize-y focus:border-accent"
              />
            </label>
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-[13px] font-semibold text-text">Składniki</span>
                <Btn variant="secondary" size="sm" onClick={addIngredient}>＋ Dodaj</Btn>
              </div>
              {form.ingredients.length === 0 && (
                <div className="p-4 text-center text-text-muted text-[13px] bg-background rounded-lg">
              ...
              <div className="grid gap-2">
                {form.ingredients.map((ing, i) => (
                  <div key={i} className="grid grid-cols-[1fr_120px_36px] gap-2 items-center">
                    <Select 
                      value={ing.product} onChange={v => updateIngredient(i, "product", v)}
                      options={products.map(p => ({ value: String(p.id), label: p.name }))}
                      placeholder="Wybierz produkt"
                    />
                    <div className="relative">
                      <input 
                        type="number" value={ing.weight_in_grams} onChange={e => updateIngredient(i, "weight_in_grams", e.target.value)}
                        placeholder="100" min="1"
                        className="w-full box-border py-2 pl-3 pr-8 text-sm border-[1.5px] border-border rounded-lg bg-surface text-text font-inherit outline-none focus:border-accent"
                      />
                      <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[11px] text-text-muted">g</span>
                    </div>
                    <button 
                      onClick={() => removeIngredient(i)} 
                      className="w-9 h-9 rounded-lg border-none bg-danger-light text-danger cursor-pointer text-base flex items-center justify-center hover:bg-[#FEE2E2]"
                    >✕</button>
                  </div>
                ))}
              </div>
              {form.ingredients.length > 0 && (() => {
                ...
                return (
                  <div className="mt-3 p-3 bg-background rounded-lg">
                    <div className="text-xs text-text-muted mb-2">Podgląd wartości odżywczych</div>
                    <MacroBar protein={n.pro} fat={n.fat} carbs={n.carb} calories={n.cal} />
                  </div>
                );
              })()}
